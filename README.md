...
Para poder sacar la media de la señal se sumo todos los valores que tiene esta, y ya luego se divide este resultado entre el numero total de las muestras de la señal tomada.\
Comenzamos definiendo un acumulador con un valor inicial a cero, ya luego se itera cada valor de la señal que tomamos, sumando cada muestra al acumulador, con esta suma se divide el valor entre el numero de muestras tomadas.\
...
Por otro lado tuvimos que sacar la desviacion estandar de esta señal, esta es la dispersion de los datos con respecto a la media tomada, una valor alto indica que los datos estan alejados a la media, y un valor bajo indica que el valor esta mas concentrado alrededor de el.\
Iniciando este calculo, comenzamos definiendo un acumulador, para cada muestra de la señal se calcula la diferencia con la media calculada y se eleva al cuadrado este resultado, luego se van sumando cada resultado que vamos obteniendo con la operacion anterior, ya con esto se divide entre el numero de muestras menos uno, por ultimo hacemos la raiz cuadrada del valor que obtenimos con las ecuaciones anteriores.
...
Este Histograma nos puede servir para:\
+ Identificar una tendencia central y dispercion de la señal.
+ Detectar posibles ruidos que tenga la señal.
+ Evaluar tendencias de la distribucion.
